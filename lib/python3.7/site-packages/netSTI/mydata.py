...
class PowerLawDD:
	...
	def __init__(self): 
		...
		self.plDistMap = np.array([0.172177071738765, 0.0849033295563347, 0.0561450712841286, 
...
class Params: 
	...
	def __init__(self): 
		...
		self.pCondom = 0.44
		self.redCondom = 0.6
		self.durI = 1 / 2 # annual scale
		self.rScr = 0.3 # annual acale
...
class SexBehavior: 
	def __init__(self, unit = 26, min = 0, max = 10, calibrate_acts = False):
		if calibrate_acts == True: 
			self.acts = np.random.uniform(min, max, 1)[0]
		else: 
			self.acts = 27 / unit
		self.dur = np.array(list(range(0, 37, 1))) # duration of (casual) relationship (months): 0 means that the relationship lasts less than a month. 
		raw_dist = np.array([0.558] + [0.216/6]*6 + [0.073/6]*6 + [0.074/12]*12 + [0.023/12]*12)
		self.dur_dist = raw_dist / np.sum(raw_dist)
