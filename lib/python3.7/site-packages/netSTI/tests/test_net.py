...
def test_random_graph_generator(): 
	...
	plt.hist(mean_deg, density=True, color = 'royalblue')
	plt.axvline(mean_deg.mean(), color = 'orangered')
	plt.text(mean_deg.mean(), 0, str(np.round(mean_deg.mean(), 3)))
	plt.xlabel('mean degree')
	plt.ylabel('density')
	plt.title(str(n_sim) + ' samples of random network')
	plt.tight_layout()
	plt.savefig('results/random mean degree.eps', format='eps', dpi=500)
	plt.clf()

	plt.figure(figsize=(6,4))
	plt.hist(cor_vec, density=True, color = 'royalblue')
	plt.axvline(cor_vec.mean(), color = 'orangered')
	plt.text(cor_vec.mean(), 0, str(np.round(cor_vec.mean(), 3)))
	plt.xlabel('correlation coefficient\nbetween degree and duration')
	plt.ylabel('density')
	plt.title(str(n_sim) + ' samples of random network')
	plt.tight_layout()
	plt.savefig('results/random cc of dd and duration.eps', format='eps', dpi=500)
	plt.clf()
...
def test_community_graph_generator(): 
	...
	plt.xlabel('mean degree')
	plt.ylabel('density')
	plt.title(str(n_sim) + ' samples of community network')
	plt.tight_layout()
	plt.savefig('results/community mean degree.eps', format='eps', dpi=500)
	plt.clf()
	...
	plt.title(str(n_sim) + ' samples of community network')
	plt.tight_layout()
	plt.savefig('results/community cc of dd and duration.eps', format='eps', dpi=500)
	plt.clf()
