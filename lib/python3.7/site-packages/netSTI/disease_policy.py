...
def make_screening_schedule(rel_hist, time_horizon, Npop, unit_per_year, pScr, corr_scr = True): 
	'''
	...
	if corr_scr == True: 
		...
		tmp_sch = np.random.binomial(1, p = pScr, size = (time_horizon, Npop))
...
def create_tracker_and_outcome_vectors(Npop, time_horizon): 
	...
	return S0, I0, T0, last_test0, person_time0, n_notification0, n_name_trace0, n_infection0, n_intervention0, \
		out_S0, out_I0, out_T0, out_newI0, out_ext_seed0, out_n_part6mon0, out_nDeliver0, \
		out_nContactTrace0, out_nNotified0, out_nTrueTreat0, out_nOvertreat0, out_nTest0, out_nScreen0, \
...
def notify_contact_deliver_sex_part(el, tmp_screen_I, pContact, unit_per_year, t, alpha = 1.0): 
	'''
	...
	n_part6mon = np.unique(screen_rel[:, :2]).shape[0] - tmp_screen_I.shape[0]
	...
	return tmp_name, tmp_count, n_part6mon
...
def contact_trace_name(n_name_trace, max_contact, p_treat_tr, last_test, unit_per_year, t): 
	...
	dum2 = (t - last_test[tmp_name]) >= (unit_per_year / 2)
	test_name = tmp_name[dum1 & dum2]
	return num_contact, tmp_name, test_name
...
def implement_strategy(ID, rel_hist, Npop, time_horizon, unit_per_year, seed_ix, \
	...
	out_S, out_I, out_T, out_newI, out_ext_seed, out_n_part6mon, out_nDeliver, out_nContactTrace, out_nNotified, out_nTrueTreat, \
	...
	for t in range(0, time_horizon):  
		...
		# excluded those who have got tested in the last 6 months
		dum = (t - last_test[scr_ID]) > (unit_per_year / 2) 
		scr_ID = scr_ID[dum]
		last_test[scr_ID] = t
		...
		if strategy == "tracing": 
			num_contact, trace_name, trace_test_name = \
			...
			tmp_name, tmp_count, n_part6mon = notify_contact_deliver_sex_part(rel_hist, tmp_screen_I, \
				pContact_tr, unit_per_year, t)
		...
		if strategy == "EPT": 			
			deliver_name, deliver_count, n_part6mon = notify_contact_deliver_sex_part(rel_hist, tmp_screen_I, \
				pContact_ept, unit_per_year, t)
			...
			dum = (t - last_test[deliver_name]) >= (unit_per_year / 2)
			deliver_name = deliver_name[dum]
		...
		if strategy == "PN": 
			...
			notify_name, tmp_count, n_part6mon = notify_contact_deliver_sex_part(rel_hist, tmp_screen_I, pContact_PN, \
				unit_per_year, t)
			dum = (t - last_test[notify_name]) >= (unit_per_year / 2)
			notify_name = notify_name[dum]
			n_notification[notify_name] = tmp_count[dum]
