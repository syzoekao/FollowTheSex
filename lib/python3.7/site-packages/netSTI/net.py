...
def power_law_graph_generator(ID, Npop, dur, dur_dist, time_horizon, n_yrs = 10): 
	...
	if n_yrs == 10: 
		...
		nPart = data.PowerLawDD().nPart30yr
		plDist = data.PowerLawDD().plDist30yr		
...
def SIR_net_generator(meanActs, run, Npop, years = 20, days = 14, 
	strategy = "null", graph = "random", adjust_sex_acts = False, calibration = False): 
	'''
	Npop = 5000
	years = 30
	...
	if graph == "random": 
		...
		rel_hist = power_law_graph_generator(ID, Npop, dur, dur_dist, time_horizon, n_yrs = years)
