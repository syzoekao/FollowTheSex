...
from netSTI.plDistHigh import plDistMap
...
def period_Relation(N, t, time_horizon): 
	...
	dur = np.array(list(range(0, 80, 1))) # duration of (casual) relationship (months): 0 means that the relationship lasts less than a month. 
	dur_dist = np.array([0.558/2]*2 + [0.216/13]*13 + [0.073/13]*13 + [0.074/26]*26 + [0.023/26]*26)
...
def k_regular_graph_generator(node, Ndegree, plDistMap, time_horizon, t_end = 1): 
	...
	for t in range(1, t_end + 1): 
		...
		if t > 1: 
			...
			for i, v in temp_for_existing.items(): 
				temp_rel_dict[i].update({key: val for key, val in v.items() if val["end"] >= t*26}) 
...
def SIR_net_generator(run, graph_function, Npop = 1000, Ndegree = 3, time_horizon = 12*5, pInf = 0.17, pCondom = 0.5, 
	...
	p_treat_tr = 0.79
	p_treat_PN = 0.71
	max_contact = 15 # maximum of contact tracing per month for the public health officials
	max_ept = 500 # maximum packs of ept delivery per person
	alpha = 1
	'''
	...
	# for t in range(0, 52): 
		...
		for x in inf_actor: 
			...
			freq = np.random.poisson(lam = 27/26, size = len(temp)) # we only sample the frequence of sex for the discordant couples to calculate infection
	...
	sum_all['newI'] = sum(out_sum["newI"][(time_horizon - 26*2 + 1):])
	...
	sum_all['avg_degree'] = sum(out_sum['avg_degree'][(time_horizon - 26*2 + 1):])/len(out_sum['avg_degree'][(time_horizon - 26*2 + 1):])
	...
	sum_all['Util'] = sum(out_sum["Util"][(time_horizon - 26*2 + 1):])
	...
	temp_dist = [monthly_degree_dist(rel_hist, Npop, x) for x in range((time_horizon - 26*2 + 1), (time_horizon + 1))]
