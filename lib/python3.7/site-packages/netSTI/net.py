	...
	for i in ID: 
		...
		if n_samp > 0: 
			...
			if d_distribution == "random":
				...
				tmp_dum = ID_cluster[include_set] == ID_cluster[i]
				p_in = (n_samp * pCluster) / np.sum(tmp_dum)
				p_out = n_samp * (1 - pCluster) / np.sum(1 - tmp_dum)
				tmp_p = tmp_dum * p_in + (1 - tmp_dum) * p_out
				if include_set.shape[0] > n_samp: 
					sampID = np.random.choice(include_set, p = tmp_p, size = n_samp, replace = False)
				else: 
					sampID = include_set				
