...
class CopulaDist(Copula): 
	...
	def get_degree_power_law(self, n_yrs): 
		if n_yrs == 10: 
			...
			x1_cdf = np.cumsum(data.PowerLawDD().plDist40yr)
			x1_nPart = data.PowerLawDD().nPart40yr
		else: 
			x1_cdf = np.cumsum(data.PowerLawDD().plDist50yr)
			x1_nPart = data.PowerLawDD().nPart50yr
