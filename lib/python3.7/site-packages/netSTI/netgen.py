...
def period_Relation(agent_ID, part_ID, dur_samp, time_horizon): 
	...
	dur_samp (numpy array): an 1D array of the relationship duration assigned to each individual 
	...
	rel_end = rel_begin + dur_samp
	return rel_begin, rel_end


def generate_edge_list_array(tmpMatrix, time_horizon, graph, independent): 
	...
	"""
	dd = np.sum(tmpMatrix, axis = 1)
	agent_ID, part_ID = np.where(tmpMatrix == 1)
	dum = agent_ID < part_ID
	agent_ID = agent_ID[dum]
	part_ID = part_ID[dum]
	el = np.vstack((agent_ID, part_ID)).T
	dur_samp = copula.CopulaDist(graph, el, dd, independent = independent).get_duration()
	rel_beg, rel_end = period_Relation(agent_ID, part_ID, dur_samp, time_horizon = time_horizon)
	out_rel = np.vstack((agent_ID, part_ID, rel_beg, rel_end)).T
	out_rel = out_rel.astype(int)
...
def graph_generator(G, ID, Npop, Ndegree, time_horizon, years, \
	...
	if G in ['random', 'community']: 
		gen_dd = copula.DegreeSequence(Npop).get_degree_poisson(Ndegree)
	if G == 'power_law': 
		gen_dd = copula.DegreeSequence(Npop).get_degree_power_law()
	if G == 'empirical': 
		gen_dd = copula.DegreeSequence(Npop).get_dd_empirical(years)
	...
	# generate edge list
	out_rel = generate_edge_list_array(tmpMatrix, time_horizon = time_horizon, graph = G, \
...
class SumNet: 
	...
	def __init__(self, el, Npop, time_horizon, t_begin): 
		self.el = el
		self.Npop = Npop
		self.time_horizon = time_horizon
		self.t_begin = t_begin

	def calc_avg_degree_per_step(self): 
		# Calculate average degree per time step
		mean_deg_per_step = np.zeros(self.time_horizon)
		p_iso = np.zeros(self.time_horizon)
		for t in range(self.time_horizon): 
			...
			p_iso[t] = 1 - tmp_ix.shape[0] / self.Npop
		return mean_deg_per_step, p_iso

	def calc_degree_over_a_window(self): 
		# calculate degree over the analysis window
		tmp_ix, tmp_n = np.unique(self.el[np.where(self.el[:, 3] > self.t_begin), :2], return_counts = True)
		out_degree = np.zeros(self.Npop)
		out_degree[tmp_ix] = tmp_n
		p_iso = 1 - tmp_ix.shape[0] / self.Npop # isolates over the evalutation window
		return out_degree, p_iso

	def sumarize_net_result(self): 
		...
		sum_d = (tmp_degree[self.el[:, 0]] + tmp_degree[self.el[:, 1]]) / 2
		cor = np.round(np.corrcoef(sum_d, g_dur)[0, 1], 5)
		mean_dur = np.mean(g_dur)

		# calculate degree over the analysis window
		tmp_deg, iso_a_window = self.calc_degree_over_a_window()
		mean_deg_a_window = np.sum(tmp_deg) / self.Npop

		# calculate the proportion of isolates in the analysis window
		tmp_out = self.calc_avg_degree_per_step()
		iso = np.mean(tmp_out[1])

		# Calculate average degree per time step
		mean_deg_per_step = np.mean(tmp_out[0])

		return {'deg_all': mean_deg_all, 'deg_a_window': mean_deg_a_window, \
		'avg_deg_step': mean_deg_per_step, 'iso_a_window': iso_a_window, 'iso_per_cycle': iso, \
		'cor': cor, 'avg_dur': mean_dur / 2}
