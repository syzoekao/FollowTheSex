...
def SIR_net_generator(run, Npop, years = 20, days = 14, 
	graph = "random", pEFoI = (1 / 5000) / 2, independent = True, base_case = True, 
	...
	count_all = ngen.SumNet(rel_hist, Npop, time_horizon, t_begin).calc_degree_over_a_window()

	seed_ix = dp.gen_inf_id_from_external(count_all, Npop, pEFoI, ID_cluster, graph, \
		time_horizon, inf_independent = True)

	scr_schedule = dp.make_screening_schedule(rel_hist, time_horizon, Npop, unit_per_year, pScr)
	...
	if base_case == True: 
		...
		param_v = [0.5, 0.6, 0.7, 0.71, 0.79, 0.8, 0.9]	
		tr_out = {}
		for v in param_v: 
